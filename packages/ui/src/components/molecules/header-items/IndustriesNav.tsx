            ...
            <ul className="grid grid-cols-3 gap-4 w-full tracking-normal ">
              {industriesItems.map((item, index) => (
                <a
                  ...
                  className={`${[1, 4].includes(index) ? "pl-[48px] text-start" : "text-start "} w-fit whitespace-nowrap paragraph text-xs hover:text-primary`}
                >
                  {item.title}
                </a>
              ))}
          ...
          <div className="flex flex-col gap-[42px]">
            {whatWeDoItems.map((item, index) => (
              <div key={index} className="grid grid-cols-3 gap-4 w-full">
                <div className="w-fit">
                  <div className="paragraph text-left font-bold text-xs pl-9">
                    {item.title}
                  </div>
                </div>
