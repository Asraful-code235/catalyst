    ...
    >
      {data?.showTitle && (
        ...
        >
          {data?.title && (
            <h2
              className={`text-[32px] lg:text-[64px] flex-shrink-0 leading-normal font-medium ${data?.rtl ? "lg:w-fit" : "lg:max-w-[720px]"} w-full`}
            >
              {data?.title}
            </h2>
          )}
          {data?.description && (
            <h2 className="description lg:max-w-[710px] w-full">
              {data?.description}
            </h2>
          )}
        </div>
